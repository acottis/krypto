...
impl BlowFish {
    pub fn init(key: &[u8]) -> Self {
        ...
        for subkey in s.subkeys.iter_mut() {
            let mut k: u32 = 0;
            for _ in 0..4 {
                k = (k << 8) | key[key_byte] as u32;
                key_byte = (key_byte + 1) % key.len();
            }
            *subkey ^= k;
        }
        ...
        for i in (0..s.subkeys.len()).step_by(2) {
            s.encrypt(&mut lhs, &mut rhs);
            s.subkeys[i] = lhs;
            s.subkeys[i + 1] = rhs;
        }

        for sbox in 0..s.sboxes.len() {
            for entry in (0..s.sboxes[0].len()).step_by(2) {
                s.encrypt(&mut lhs, &mut rhs);
                s.sboxes[sbox][entry] = lhs;
                s.sboxes[sbox][entry + 1] = rhs;
            }
        }
    ...
    fn encrypt(&self, lhs: &mut u32, rhs: &mut u32) {
        for round in 0..16 {
            *lhs ^= self.subkeys[round];
            *rhs ^= self.f(*lhs);
            swap(lhs, rhs);
        }
        swap(lhs, rhs);
    ...
    fn decrypt(&self, lhs: &mut u32, rhs: &mut u32) {
        for round in (2..self.subkeys.len()).rev() {
            *lhs ^= self.subkeys[round];
            *rhs ^= self.f(*lhs);
            swap(lhs, rhs);
        }
    ...
    fn f(&self, mut lhs: u32) -> u32 {
        let d = self.sboxes[3][lhs as usize & 0xFF];
        lhs >>= 8;
        let c = self.sboxes[2][lhs as usize & 0xFF];
        lhs >>= 8;
        let b = self.sboxes[1][lhs as usize & 0xFF];
        lhs >>= 8;
        let a = self.sboxes[0][lhs as usize & 0xFF];
    ...
    pub fn encrypt_block(&self, block: &[u8; 8]) -> u64 {
        let mut lhs = (block[0] as u32) << 24
            ...
            | (block[2] as u32) << 8
            | (block[3] as u32) << 0;
        let mut rhs = (block[4] as u32) << 24
            | (block[5] as u32) << 16
            | (block[6] as u32) << 8
            | (block[7] as u32) << 0;

        self.encrypt(&mut lhs, &mut rhs);
    ...
    pub fn decrypt_block(&self, block: &[u8; 8]) -> u64 {
        ...
        self.decrypt(&mut lhs, &mut rhs);
        (lhs as u64) << 32 | rhs as u64
    }
}

#[inline(always)]
fn swap(lhs: &mut u32, rhs: &mut u32) {
    let tmp = *lhs;
    *lhs = *rhs;
    *rhs = tmp;
}
...
mod tests {
    ...
    #[test]
    fn test_blowfish_init() {
        let key = [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0];

        let bl = BlowFish::init(&key);
        assert_eq!(
            bl.subkeys,
            [
                0x706D9FCC, 0x1792D23A, 0x2DB9D714, 0x966E1439, 0xAC21A76D,
                0x8324E988, 0xAC0DC9DD, 0x2C38F6B3, 0x70619520, 0xFA23ECBE,
                0x17B2F676, 0xEBA13A04, 0x8B949E61, 0x7A147CAF, 0x56CCC6B6,
                0x4461B24D, 0x7361E6A1, 0x196A7C43
            ]
        )
    }
    #[test]
    fn test_blowfish_encrypt() {
        let key = [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0];

        let bf = BlowFish::init(&key);
        let ciphertext = bf.encrypt_block(&key);
        assert_eq!(ciphertext, 0x4ef997456198dd78)
    }

    #[test]
    fn test_blowfish_decrypt() {
        let key = [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0];

        let bf = BlowFish::init(&key);
        let mut ciphertext = u64::to_be_bytes(0x4ef997456198dd78);
        let plaintext = bf.decrypt_block(&ciphertext);
        assert_eq!(plaintext, 0x00);
    }

    #[test]
    fn test_blowfish_vectors() {
        let test_cases: [(u64, u64, u64); 34] = [
        ...
        for (key, clear, cipher) in test_cases {
            let bf = BlowFish::init(&u64::to_be_bytes(key));
            let res = bf.encrypt_block(&u64::to_be_bytes(clear));
            assert_eq!(
                cipher, res,
                "Case Key: {key:X}, Text: {clear:X}, Recv Cipher: {res:X}, Actual Cipher: {cipher:X}"
            );
        }
    }
}
